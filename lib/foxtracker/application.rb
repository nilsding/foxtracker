...
module Foxtracker
  ...
  class ExtendedModule < Dry::Struct
    ...
    class Sample < Dry::Struct
      ...
      attribute :packing_type, Types::Strict::String.enum("delta" => 0, "adpcm" => 0xAD)
    ...
    class Instrument < Dry::Struct
      ...
      attribute :volume_type, Types::Strict::String.enum("on" => 0, "sustain" => 1, "loop" => 2).meta(omittable: true)
      attribute :panning_type, Types::Strict::String.enum("on" => 0, "sustain" => 1, "loop" => 2).meta(omittable: true)
    ...
    def self.parse(bin)
      ...
      args[:title] = bin[offset...(offset += 20)].strip
      raise "invalid XM module" unless bin[offset...(offset += 1)] == "\x1A"

      args[:tracker] = bin[offset...(offset += 20)].strip
    ...
    def self.parse_instruments(bin, xm_args, offset)
      ...
      xm_args[:number_of_instruments].times do
        instruments << {}.tap do |instrument_args|
          # 1st part
          instrument_args[:header_size] =
            bin[offset...(offset += 2)].unpack1("S<") << 8 | bin[offset...(offset += 2)].unpack1("S<")
          instrument_args[:name] = bin[offset...(offset += 22)].strip
          instrument_args[:type] = bin[offset...(offset += 1)].unpack1("C")
          instrument_args[:number_of_samples] = bin[offset...(offset += 2)].unpack1("S<")
          next if instrument_args[:number_of_samples].zero?

          # 2nd part
          instrument_args[:sample_header_size] =
            bin[offset...(offset += 2)].unpack1("S<") << 8 | bin[offset...(offset += 2)].unpack1("S<")
      ...
      instrument_args[:number_of_samples].times do
        samples << {}.tap do |sample_args|
          sample_args[:sample_length] =
            bin[offset...(offset += 2)].unpack1("S<") << 8 | bin[offset...(offset += 2)].unpack1("S<")
          sample_args[:sample_loop_start] =
            bin[offset...(offset += 2)].unpack1("S<") << 8 | bin[offset...(offset += 2)].unpack1("S<")
          sample_args[:sample_loop_length] =
            bin[offset...(offset += 2)].unpack1("S<") << 8 | bin[offset...(offset += 2)].unpack1("S<")
    ...
    def self.unpack_sample_data(sample_args)
      ...
      packed_samples = sample_args[:raw_data].unpack(unpack_str)
